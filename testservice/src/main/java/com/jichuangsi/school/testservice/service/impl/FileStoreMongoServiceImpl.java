...
public class FileStoreMongoServiceImpl implements IFileStoreService {
    ...
    @Override
    public TestFile uploadTestFile(TestFile file) throws Exception{
        ObjectId objectId = gridFsOperations.store(new ByteArrayInputStream(file.getContent()), file.getStoredName(), file.getContentType());
        return file;
    }

    @Override
    public TestFile downloadTestFile(String fileName) throws Exception{
        ...
        if(result != null){
            ...
            TestFile downloadFile = new TestFile();
    ...
    @Override
    public void deleteTestFile(String fileName) throws Exception {
        GridFSFile result = gridFsOperations.findOne(new Query(GridFsCriteria.whereFilename().is(fileName)));
        if(result != null){
            try{
