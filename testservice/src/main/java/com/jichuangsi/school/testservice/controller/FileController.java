...
import com.jichuangsi.school.testservice.model.common.Base64TransferFile;
...
public class FileController {
    ...
    @ApiOperation(value = "上传test的附件", notes = "")
    @ApiImplicitParams({
    ...
    public ResponseModel<Attachment> saveAttachment(@RequestParam MultipartFile file, @ModelAttribute UserInfoForToken userInfo) throws FileServiceException{
        try {
            return ResponseModel.sucess("", fileService.uploadAttachment(userInfo, new TestFile(file.getOriginalFilename(), file.getContentType(), file.getBytes())));
    ...
    @ApiOperation(value = "删除test的附件", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "header", name = "accessToken", value = "用户token", required = true, dataType = "String")
    })
    ...
    public void getAttachment(@ModelAttribute UserInfoForToken userInfo, @RequestBody Attachment attachment, HttpServletResponse resp) throws FileServiceException{
        try {
            TestFile file = fileService.downloadFile(userInfo, attachment.getSub());
    ...
    public ResponseModel<String> sendByFile(@RequestParam MultipartFile file, @ModelAttribute UserInfoForToken userInfo) throws FileServiceException {
        try {
            return ResponseModel.sucess("", fileService.uploadPic(userInfo, new TestFile(file.getOriginalFilename(), file.getContentType(), file.getBytes())));
    ...
    public ResponseModel<String> sendByString(@ModelAttribute UserInfoForToken userInfo, @RequestBody Base64TransferFile file) throws FileServiceException {
        return ResponseModel.sucess("", fileService.uploadPic(userInfo, new TestFile(file.getName(), file.getContentType(), file.getContent().getBytes())));
    }
    ...
    public ResponseModel<Base64TransferFile> getByString(@ModelAttribute UserInfoForToken userInfo, @RequestBody String sub) throws FileServiceException {
        ...
        TestFile testFile = fileService.downloadFile(userInfo, JSON.parseObject(sub).getString("sub"));
        base64TransferFile.setName(testFile.getName());
        base64TransferFile.setContentType(testFile.getContentType());
        base64TransferFile.setContent(new String(testFile.getContent()));
    ...
    public ResponseModel<TestFile> getByFile(@ModelAttribute UserInfoForToken userInfo, @RequestBody String sub) throws FileServiceException {
