...
public class FeignStaticsServiceImpl implements FeignStaticsService {
    ...
    public List<Question> getQuestionBySubjectId(String subjectId) {
      ...
      /*List<Question> list1 =questionRepository.findOnMonthQuestionBySubjectIdAndCreateTimeIsAfter(subjectId,calendar.getTimeInMillis());

        List<Question> list =questionRepository.findBySubjectIdAndCreateTimeGreaterThan
                                                 (subjectId,calendar.getTimeInMillis());*/

        //mongoTemplate.find(new Query(Criteria.where("subjectId").is(subjectId)),Question.class);
    ...
    public Question getQuestionKnowledges(String questionId) {
      //  Question q= mongoTemplate.findOne(new Query(Criteria.where("id").is(questionId)),Question.class);
        //System.out.println(q.getKnowledges().size());
